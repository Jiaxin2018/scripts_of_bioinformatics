...
class TransformFrame extends JFrame implements ActionListener {
	...
	public TransformFrame() throws Exception {
...
class TransPanel extends JPanel {
	...
	int zoomLevel = 0;
	public TransPanel() throws Exception {
		// 首先载入一张图片。
		img = ImageIO.read(new File("D:\\pcrbmp.bmp"));}
	
	public void paintComponent(Graphics g) {
		super.paintComponent(g);
		//g.drawImage(img, 0, 0, this);
		//drawTransImage(g, img.getWidth(this), img.getHeight(this), zoomLevel);
		drawTransImage(g,150, 50, zoomLevel);
		}
	public void drawTransImage(Graphics g, int drawx, int drawy, int zoom) {
		...
		// 上面的m_nFlipXScale * m_nFlipYScale需要特殊说明一下：因为实际使用中，可能遇到各种组合的情况，比如先flipX或者flipY以后然后再旋转，这时候，图片的旋转方向就会出现错误，加上这段代码可以保证无论使用哪种组合操作方式，都保证在旋转图片的时候是按照顺时针的方向进行旋转。
		if (m_nFlipXScale == -1)
